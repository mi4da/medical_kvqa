...
if __name__ == '__main__':
    ...
    train_dataset = SlakeVQADataset(json_file=train_json_file_path,
                                    img_dir=img_dir_path, transform=transform)
    ...
    vqa_model = VQAModel(768, 768, 768, num_classes=num_classes).to(
        device)  # Replace num_classes with actual number of classes
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(tqdm.tqdm(train_loader, desc=f"Epoch [{epoch + 1}/{num_epochs}]")):
            ...
            # 将gt转换为整数标签
            answer = torch.tensor(label_encoder.transform(batch['answer'])).to(device)
            ...
            tail = batch['tail']
            head_embedding = query_KG(base_type, relation, tail, kg_embedding).to(device)
        ...
        if epoch % 1 == 0:
            ...
            with torch.no_grad():  # 禁用梯度计算
                for batch in tqdm.tqdm(val_loader, desc="Validation"):
                    image = batch['image']
                    question = batch['question']
                    answer = torch.tensor(label_encoder.transform(batch['answer']), dtype=torch.long).to(device)
